export default function resultCard(result) {
  if (result.type === 'person') {
    const { name, profile, knownFor, job } = result;
    const knownForYear = new Date(
      knownFor.release_date || knownFor.first_air_date,
    ).getFullYear();

    return `<li class="result-card person">
        <img
          src=${profile}
          alt="${name} poster"
          class="result-poster"
        ...
        <div class="result-details">
          ...
          <a href="#" class="known-for">
            <span>${knownFor.name || knownFor.title}</span>(<span>${knownForYear}</span>)
          </a>
        </div>
